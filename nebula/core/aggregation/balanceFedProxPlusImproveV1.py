...
# New Improve
# Integrated with FedAvg weights.
class BalanceFedProxPlusImproveV1(Aggregator):
    ...
    def run_aggregation(self, models):
        ...
        prox_center = self.compute_prox_center(
            {**filtered_models, self._addr: (local_model, 0)}
        )
        ...
        SW = sum(w for _, w in list(filtered_models.values()))
        W = sum(w for _, w in list(models.values())) - models[self._addr][1]

        accum = {layer: torch.zeros_like(param) for layer, param in local_model.items()}

        with torch.no_grad():
            # Weighted average
            for addr in filtered_models:
                params, soft_w = filtered_models[addr]
                _, w = models[addr]
                for layer in accum:
                    accum[layer] += params[layer] * (soft_w / SW + w / W) * 0.5
