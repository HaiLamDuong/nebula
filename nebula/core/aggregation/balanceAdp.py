...
class BalanceAdp(Aggregator):
    def __init__(self, config=None, **kwargs):
        super().__init__(config, **kwargs)
        self._local_model = None
        # self.round = self.engine.round
        self._prev_smoothed = None
        self.A = 1.5          # Threshold scale factor
        self.K = 1.0          # Decay rate
        self.local_weight = 0.2  # Thêm hyperparameter
        logging.info(f"[{self.__class__.__name__}] Init BalanceAdp - Hybrid Balance + FedAdp")
    ...
    def remove_malicious_models(self, models, local_model):
        """
    ...
    def smooth_cosine_similarity(self, similarities):
        ...
        if self._prev_smoothed is None:
            self._prev_smoothed = {}
            logging.debug(f"[smooth_cosine_similarity] previous smoothed is None at round {self.engine.round}")
    ...
    def run_aggregation(self, models):
        ...
        filterd_models = self.remove_malicious_models(models, local_model)

        if not filterd_models:
            logging.warning(f"[{self.__class__.__name__}] No models passed filtering, returning local model")
            return local_model

        # init weight for each client
        weights = {}
        for addr, (_, weight) in filterd_models.items():
            weights[addr] = weight


        similarities = self.calculate_cosine_similarity(filterd_models, local_model)
        ...
        with torch.no_grad():
            for node_addr, (model_parameters, _) in filterd_models.items():
                if node_addr == self._addr:
                    continue
        ...
        alpha = self.local_weight
        for layer in accum:
            accum[layer].mul_(1-alpha)
            accum[layer].add_(local_model[layer], alpha=(alpha))

        logging.info(f"[{self.__class__.__name__}] ========== BalanceAdp Aggregation Round Completed ==========")
        gc.collect()
        return accum
