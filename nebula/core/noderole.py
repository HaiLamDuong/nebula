...
class TrainerAggregatorRoleBehavior(RoleBehavior):
    ...
    async def extended_learning_cycle(self):
        ...
        w_agg_params = None
        if hasattr(self._engine.aggregator, "d2b_node"):
            w_agg_params = self._engine.trainer.get_model_parameters()
        ...
        if hasattr(self._engine.aggregator, "d2b_node") and w_agg_params is not None:
            w_train_params = self._engine.trainer.get_model_parameters()
            current_loss = self._engine.trainer.get_current_loss()
            safe_model, _ = self._engine.aggregator.d2b_node.local_process(
                W_train_dict=w_train_params,
                W_agg_dict=w_agg_params,
                current_loss=current_loss
            )
            self._engine.trainer.set_model_parameters(safe_model)
            self._engine.aggregator.d2b_node.post_round_update(current_loss)
